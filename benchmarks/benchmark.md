...
- 测试指标：NFE（前向次数）、TPS（每秒token数）、TPF（每次前向的token数）、FPS（每秒前向次数）
...
- 从JSON数据集加载多个样本进行批量测试
- 支持批处理（batch processing）
- 使用bucket机制（bucket_size=32）对输入长度进行分组优化
- 支持多种缓存策略（prefix、dual）
- 支持多种扩散模型变体（BlockWise、IterSmooth、VicinityCache等）
- 输出详细的结果到JSONL文件和 results.txt
...
| 特性 | benchmark.py | benchmark_dataset.py | benchmark_dataset_fastdllm.py | benchmark_dataset_sglang.py | benchmark_dataset_sorted.py |
|------|-------------|---------------------|------------------------------|----------------------------|----------------------------|
| 测试类型 | 单样本 | 批量数据集 | 批量数据集 | 批量数据集 | 批量数据集 |
| 后端框架 | vLLM | vLLM | vLLM | SGLang | vLLM |
| 生成方法 | 扩散模型 | 扩散模型 | FastDLLM | 扩散模型 | 扩散模型 |
| 输入排序 | 否 | 否 | 否 | 是 | 是 |
| 动态预热 | 否 | 否 | 否 | 否 | 是 |
| Bucket大小 | N/A | 32 | 8 | 32 | 32 |
| 批处理 | 否 | 是 | 否（固定为1） | 是 | 是 |
| 配置预设 | 否 | 是（1-15, 40-41） | 否 | 是（1-15, 40-41） | 是（1, 40） |
...
### 解码策略
- `--parallel_decoding`: 解码策略，可选 `threshold`、`hierarchy_faster`
